...
export interface SyrianLogoAnimationProps {
  className?: string;
  size?: 'sm' | 'md' | 'lg' | 'xl';
  animated?: boolean;
  color?: 'gold' | 'green' | 'maroon' | 'white';
}

/**
 * Animated Syrian Logo component using the new golden SVG logo
 */
export const SyrianLogoAnimation: React.FC<SyrianLogoAnimationProps> = ({
  className = '',
  size = 'md',
  animated = true,
  color = 'gold'
  ...
  // Animation variants for the inner elements (stars)
  const starVariants = {
  ...
  const SVGContent = () => (
    <svg viewBox="0 0 512 512" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Main logo path */}
      <motion.path
        fill={color === 'white' ? '#ffffff' : `url(#${color}Gradient)`}
        filter="url(#shadow)"
...
/**
 * Pulsating Syrian Logo for loading states
 */
export const PulsingSyrianLogo: React.FC<SyrianLogoAnimationProps> = ({
  className = '',
  size = 'md',
  color = 'gold'
}) => {
...
export default SyrianLogoAnimation;